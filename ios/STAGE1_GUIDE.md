...
6. **即時資料**
   - `SensorViewModel`：訂閱 `readings` 的 `INSERT` Realtime 更新 `currentReading`。
   - 歷史查詢使用集中設定（如 `Constants.History` 或設定檔）定義時間窗、取樣間隔與筆數上限，避免程式散佈 magic numbers。
7. **介面**
   - `LoginView`：負責登入/註冊流程。
   - `DashboardView`：顯示最新溫/濕度卡片；預設 Temp>30 或 Humidity>70 觸發震動/通知，門檻集中於設定/常數（可用 UserDefaults 覆寫）。
   - `HistoryChartView`：Swift Charts 雙折線，使用歷史查詢結果繪圖。
8. **驗證**
