...
3. **感測初始化**：在 `setup()` 以 GPIO 15 建立 DHT11，序列埠輸出啟動狀態。定義週期：標準模式 5 分鐘、Demo 模式 10 秒。
4. **讀值與監控**：在 `loop()` 先檢查 Wi-Fi，斷線即自動重連。讀取 DHT11，若溫濕度為 NaN 直接跳過並記錄。
5. **資料上傳**：使用 `HTTPClient` POST 至 `<SUPABASE_URL>/rest/v1/readings`，Header 包含 `apikey` 與 `Authorization: Bearer <anon key>`，Body 為含 `device_id/temperature/humidity` 的 JSON 物件。設定 `Content-Type: application/json` 與 `Prefer: return=minimal`。
